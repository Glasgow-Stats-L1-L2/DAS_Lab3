...
From here select **Document** and the **Default Output Format** as **HTML**. Give your document a title and select OK. This will open within RStudio an empty `.qmd` file.

::: callout-note
There is a template on the DAS Moodle page which you can download to get started. To see what the HTML/PDF of the template document looks like click on the ![](images/rstudio-render-button.png){width="16" height="13"} `Render` button at the top of the document window. You can also download this [template](https://quarto.org/docs/get-started/hello/rstudio/_hello.qmd) from the quarto webpage containing simple instructions on how to do some basic stuff using Quarto .
:::
